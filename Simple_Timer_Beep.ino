...
int controlPin=12;          // Номер контакта для управляющего сигнала на реле (пока для контроля - светодиод)
unsigned long timer;
int buttonPressed;
byte c_ya[8] = {
  B01111,
...
// Переменные для настройки звукового сигнала
// Включение звукового сигнала по окончании отсчета времени
  bool BEEP_ON = true;
// Контакт на который подключен звуковой излучатель (динамик)
  int BEEP_PIN = 11;
// Длительность звукового сигнала в миллисекундах
  int BEEP_DUR = 1500;
...
void loop() {
  ...
  switch (buttonPressed) {
    ...
    case KEYPAD_SELECT:                                         // Нажата кнопка "Start/Pause"
        ...
        if (BEEP_ON) {                              // если включено, то подаем звуковой сигнал
          tone(BEEP_PIN, BEEP_TON);
          delay (BEEP_DUR);
          noTone(BEEP_PIN);
        }
      break;
